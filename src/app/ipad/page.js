...
const HomeIpad = () => {
  ...
  useEffect(() => {
    if (state === 3) {
      ...
      fetch('https://braftovi-interactive-website-hy2310107.vercel.app/api/record', {
        headers: {
  ...
  const createUser = async (val) => {
    const result = await fetch('https://braftovi-interactive-website-hy2310107.vercel.app/api/user', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
